...
export class WebsocketServer extends DurableObject {
    ...
    while (unmatchedGames.size > 0 && 
      ...
      try {
        ...
        for (const [gameKey, gameData] of unmatchedGames.entries()) {
          ...
          const qualifierTimeISO = this.convertQualifierTimeToISO(apiDate, game.qualifiedTime);
          if (!qualifierTimeISO) {
  ...
  findEarliestQualifierTime(qualifiedGames) {
    ...
    console.log(`Date string for qualifier time search: ${dateString}`);
    ...
    for (const game of qualifiedGames) {
      ...
      try {
        const isoTime = this.convertQualifierTimeToISO(dateString, game.qualifiedTime);
        if (!isoTime) {
          console.error(`Failed to convert time ${game.qualifiedTime} to ISO format`);
          continue;
        }
        
        const gameQualifierDate = new Date(isoTime);
  ...
  matchesTeam(oddsApiTeamName, gameTeamName) {
    ...
    // Clean up team names for comparison - just do basic normalization
    const cleanOddsName = oddsApiTeamName.trim().toLowerCase()
      .replace(/\s+/g, ' ');     // Normalize whitespace
      
    const cleanGameName = gameTeamName.trim().toLowerCase()
      .replace(/\s+/g, ' ');     // Normalize whitespace
    
    // Simple exact string comparison
    const isMatch = cleanOddsName === cleanGameName;
    
    if (!isMatch) {
      console.log(`Team name mismatch: "${oddsApiTeamName}" !== "${gameTeamName}"`);
    }
